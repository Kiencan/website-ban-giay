...
function renderPagination(totalPages, currentPage) {
    ...
    if (totalPages <= 7) {
        ...
        if (currentPage <= 4) {
            ...
            pageArray = [1, '...', totalPages - 4, totalPages - 3, totalPages - 2, totalPages - 1, totalPages];
        } else {
            pageArray = [1, '...', currentPage - 1, currentPage, currentPage + 1, '...', totalPages];
        }
    }
    ...
    pageArray.forEach(page => {
        if (page === '...') {
            paginationLinks.append(`<a href="#" class="btn rounded disabled">...</a>`);
        } else if (page === currentPage) {
            paginationLinks.append(`<a href="#" class="btn rounded active">${page}</a>`);
        } else {
            paginationLinks.append(`<a href="#" class="btn rounded" data-page="${page}">${page}</a>`);
        }
    });
