...
if (isPost()) {
    ...
    if (empty($errors)) {
        ...
        $dataUpdate['product_image'] = basename($image);
        $imagePath = $targetDir . $imageDetail['product_image'];
        $updateStatus = update('product_image', $dataUpdate, "image_id = '$image_id'");
        if ($updateStatus) {
            if (move_uploaded_file($imageTemp, $targetFilePath)) {
                if (file_exists($imagePath)) {
                    unlink($imagePath);
                    setFlashData('smg', 'Cập nhật hình ảnh sản phẩm thành công!');
                    setFlashData('smg_types', 'success');
                    redirect('?module=admin&action=product_image_edit&p_id=' . $p_id . '&image_id=' . $image_id);
                } else {
                    setFlashData('smg', 'Upload ảnh không thành công!');
                    setFlashData('smg_types', 'danger');
                    redirect('?module=admin&action=product_image_edit&p_id=' . $p_id);
                }
            } else {
    ...
    <div id="page-content-wrapper">
        <div class="container-fluid px-4 pt-3 border">
            <ul class="breadcrumb">
                ...
                <li class="breadcrumb-item active"> Cập nhật hình ảnh sản phẩm </li>
            </ul>
        </div>

        <div class="container-fluid px-4">
            <h1 class="mt-4">Cập nhật hình ảnh sản phẩm</h1>
        </div>
